...
const STORAGE = "atc_quiz_full_v1";
let state = {
  mode: "sequential",
  order: [],
  index: 0,
...
function getRange(){
  let start = clamp(parseInt($("startNo").value||1),1,ANSWERS.length);
  let end = clamp(parseInt($("endNo").value||ANSWERS.length),1,ANSWERS.length);
...
function startQuiz(){
  const s = loadStore();
  let order = [];
  if(state.mode==="sequential") order = Array.from({length:ANSWERS.length},(_,i)=>i+1);
  if(state.mode==="range") order = getRange();
  if(state.mode==="random") order = shuffle(getRange());
  if(state.mode==="mock"){
    const count = clamp(parseInt($("mockCount").value||50),1,ANSWERS.length);
  ...
  state.order=order; state.index=0; state.sessionAnswers={}; state.correct=0; state.startedAt=Date.now();
...
function render(){
  ...
  $("questionText").textContent = "請選擇 A、B、C、D";
  $("feedback").className = "feedback";
  $("feedback").textContent = answered ? (answered.choice===ANSWERS[qNo-1] ? "正確！" : `錯誤，正確答案：${ANSWERS[qNo-1]}`) : "";
  const choices = ["A","B","C","D"].map(c=>{
    let cls="choice";
    if(answered){
      if(c===ANSWERS[qNo-1]) cls+=" correct";
...
function choose(choice){
  ...
  const correctAnswer = ANSWERS[qNo-1];
  const ok = choice===correctAnswer;
  state.sessionAnswers[qNo] = {choice, ok};
  if(ok) state.correct++;
  const s = loadStore();
  s.total++; if(ok) s.correct++;
  if(ok) delete s.wrong[qNo]; else s.wrong[qNo] = {answer: correctAnswer, picked: choice, time: new Date().toISOString()};
  saveStore(s);
  updateStats();
  render();
}
...
function showAnswer(){
  ...
  $("feedback").textContent = `正確答案：${ANSWERS[qNo-1]}`;
}
function finish(){
  if(state.timerId) clearInterval(state.timerId);
